...
public class TurnFireOn : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.E) && lastFloorPuzzle.firstBrazier && canLitUp && magicScript.canBurn && magicScript.learnedFireSpell)
        {
            fireEffect.SetActive(true);
            lastFloorPuzzle.counter++;
            lastFloorPuzzle.firstBrazier = false;
            lastFloorPuzzle.secondBrazier = true;
        }
    }
